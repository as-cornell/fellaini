<?php 
  ...
  //   if (!empty($content['field_topic'][$i]['#markup'])){
  //   $topicclass = $topicclass . " category-" . drupal_html_class($content['field_topic'][$i]['#markup']);
  //       $i++;
  //     }
  //     }
  //   $topicclass = trim(str_replace('-amp-', '-and-', $topicclass));
?>
<article class="as-card as-card--article <?php //print $topicclass; ?><?php if (empty($content['field_image'])): ?>as-card--article--noImage<?php endif; ?>" id="<?php print $title; ?>" aria-labelledby="article-title-<?php print $node->nid?>">
